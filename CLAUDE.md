...
- The `va_listen_timeout` script (8 s, `mode: restart`) recovers sessions stuck in `STREAMING_MICROPHONE`. The `on_stt_end`, `on_error` and `on_end` triggers stop it. If it fires, it shows FAILED with `timeout` and then returns to Ready.
...
- **Use `on_end`, not `on_idle`, to reset the display.** In 2026.9.0, `on_idle` only fires in continuous mode (`start_continuous`), never after `voice_assistant.start`. `on_end` waits until `voice_assistant.is_running` is false, so TTS playback has finished, then waits 1.5 s and returns to Ready only if no new session has started.
